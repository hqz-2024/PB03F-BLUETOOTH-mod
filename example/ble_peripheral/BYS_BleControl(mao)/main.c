...
#define   BLE_MAX_ALLOW_PKT_PER_EVENT_TX        3//配置连接间隔最大发包数量
#define   BLE_MAX_ALLOW_PKT_PER_EVENT_RX        3//连接间隔最大收包数量
...
*/
#if ( HOST_CONFIG & OBSERVER_CFG )//配置 sbm osal memory 空间，可参考 Link buffer 的文档介绍
...
static void hal_low_power_io_init(void)
{
    ...
    ioinit_cfg_t ioInit[]=
    {
        ...
        {GPIO_P09,   GPIO_PULL_UP_S    },/*UART0 TX*/
        {GPIO_P10,   GPIO_PULL_UP    },/*UART0 RX*/
        {GPIO_P11,   GPIO_PULL_DOWN  },
        {GPIO_P14,   GPIO_PULL_DOWN  },
        {GPIO_P15,   GPIO_PULL_DOWN  },//Restore
    ...
    #else
    hal_pwrmgr_RAM_retention(RET_SRAM0|RET_SRAM1|RET_SRAM2);//启用保留SRAM（静态随机存取存储器）
    #endif
...
//    }
    ...
    LOG("[REST CAUSE] %d\n ",g_system_reset_cause);//启动原因 	RSTC_WARM_UP = 0,上电复位  1,狗复位
    app_main();
}
