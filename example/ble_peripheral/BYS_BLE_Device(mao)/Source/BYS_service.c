...
/*********************************************************************
    GLOBAL VARIABLES
*/
uint8 bleuart_ServiceUUID[ATT_BT_UUID_SIZE] ={  LO_UINT16(AT_SERVICE_UUID_DEF), HI_UINT16(AT_SERVICE_UUID_DEF)};
// Characteristic PT uuid
uint8 bleuart_PTCharUUID[ATT_BT_UUID_SIZE] ={  LO_UINT16(AT_PT_CHAR_UUID_DEF), HI_UINT16(AT_PT_CHAR_UUID_DEF)};
...
unsigned short BYS_Service_connHandle = 0xFFFF;
/*********************************************************************
...
*/
bStatus_t BYS_BLE_SERVICE_AddService( bleuart_ProfileChangeCB_t cb){
                                      ...
                                      uint8* pValue, uint8 len, uint16 offset )
{
    ...
    if ( osal_memcmp(pAttr->type.uuid, clientCharCfgUUID, 2) )
    {
        ...
        if ( status == SUCCESS && bleuart_AppCBs)
        {
            ...
            LOG("CCCD set: [%d]\n", charCfg);
						if(charCfg){BYS_Service_connHandle = connHandle;}else{BYS_Service_connHandle = 0xFFFF;}
...
uint8 bleuart_NotifyIsReady(void){    return (bleuart_TxCCCD.value == GATT_CLIENT_CFG_NOTIFY);}
void set_Bleuart_Notify(void){    bleuart_TxCCCD.value |= GATT_CLIENT_CFG_NOTIFY;}
void clear_Bleuart_Notify(void){    bleuart_TxCCCD.value = 0;}
...
/*********************************************************************
    ...
    @return      Success or Failure
*/
bStatus_t bleuart_Notify(attHandleValueNoti_t* pNoti, uint8 taskId ){
    ...
    if ( value & GATT_CLIENT_CFG_NOTIFY ){
        ...
        //AT_LOG("N_1\n");
        return GATT_Notification( BYS_Service_connHandle, pNoti, FALSE);// Send the Indication
    }
