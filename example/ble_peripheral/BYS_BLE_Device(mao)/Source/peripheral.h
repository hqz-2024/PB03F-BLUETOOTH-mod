...
*/
typedef enum{
    GAPROLE_INIT = 0,                       //!<0 等待启动 Waiting to be started
    GAPROLE_STARTED,                        //!<1 已启动但未广播 Started but not advertising
    GAPROLE_ADVERTISING,                    //!<2 目前正在广告 Currently Advertising
    GAPROLE_WAITING,                        //!<3 设备已启动但未进行广告推送，正处于再次广告推送前的等待期 Device is started but not advertising, is in waiting period before advertising again
    GAPROLE_WAITING_AFTER_TIMEOUT,          //!<4 设备刚因连接超时，但尚未开始广告推送，正处于再次广告推送前的等待期Device just timed out from a connection but is not yet advertising, is in waiting period before advertising again
    GAPROLE_CONNECTED,                      //!<5 在连接中 In a connection
    GAPROLE_CONNECTED_ADV,                  //!<6 在连接+广告中 In a connection + advertising
