...
#define DEFAULT_MIN_CONN_INTERVAL     0x0048  // 90 milliseconds
...
#define MIN_CONN_INTERVAL             0x000C
...
bStatus_t GAPRole_SetParameter( uint16 param, uint8 len, void* pValue )
{
    ...
    case GAPROLE_ADVERT_DATA:
        if ( len <= B_MAX_ADV_LEN ){
            ...
            ret = GAP_UpdateAdvertisingData( gapRole_TaskID,TRUE, len, gapRole_AdvertData );
        }else{
            ret = bleInvalidRange;
        }
...
{
	GAP_LOG("-->%s  gapRole_state:%04X\r\n",__func__,gapRole_state);
...
void GAPRole_Init( uint8 task_id )
{
    ...
    gapRole_AdvChanMap = GAP_ADVCHAN_ALL;//GAP_ADVCHAN_37;// 
...
static void gapRole_ProcessOSALMsg( osal_event_hdr_t* pMsg ){
    ...
    switch ( pMsg->event )
    {
    case HCI_GAP_EVENT_EVENT://0x91 //!<GAP事件消息 GAP Event message
        ...
        if ( pMsg->status == HCI_COMMAND_COMPLETE_EVENT_CODE ){
            ...
            if ( pPkt->cmdOpcode == HCI_READ_RSSI ){
                ...
                if ( (gapRole_state == GAPROLE_CONNECTED) && (rssi != RSSI_NOT_AVAILABLE) ){
                    // Report RSSI to app
                    if ( pGapRoles_AppCGs && pGapRoles_AppCGs->pfnRssiRead ){pGapRoles_AppCGs->pfnRssiRead( rssi );}
                }
            ...
            if ( pPkt->cmdOpcode == HCI_LE_SET_PHY ){LOG("[HCI PHY] %2x \n",pPkt->cmdStatus);}
...
//               AT_LOG("[HCI DLE EVT]%d %d %d %d\n",pkt->MaxRxOctets,pkt->MaxRxTime,pkt->MaxTxOctets,pkt->MaxTxTime);
...
static void gapRole_ProcessGAPMsg( gapEventHdr_t* pMsg ){
    ...
    if( LegacyAdvEnable )
    {
        ...
        switch ( pMsg->opcode ){
        case GAP_DEVICE_INIT_DONE_EVENT:{//0x00 //!<在设备初始化完成时发送
					LOG("-->[ GAP_DEVICE_INIT_DONE_EVENT ]  pMsg->opcode:%02X\r\n",pMsg->opcode);
            ...
            if ( stat != SUCCESS ){gapRole_state = GAPROLE_ERROR;}
            notify = TRUE;
        }
        break;

        case GAP_ADV_DATA_UPDATE_DONE_EVENT:{//0x02 //!<当广告数据或SCAN_RSP数据已更新时发送
					LOG("-->[ GAP_ADV_DATA_UPDATE_DONE_EVENT ]  pMsg->opcode:%02X\r\n",pMsg->opcode);
            gapAdvDataUpdateEvent_t* pPkt = (gapAdvDataUpdateEvent_t*)pMsg;
            if ( pPkt->hdr.status == SUCCESS ){
                if ( pPkt->adType ){
                    // Setup the Response Data
                    pPkt->hdr.status = GAP_UpdateAdvertisingData( gapRole_TaskID,FALSE, gapRole_ScanRspDataLen, gapRole_ScanRspData );
                }else if ( ( gapRole_state != GAPROLE_ADVERTISING ) &&( osal_get_timeoutEx( gapRole_TaskID, START_ADVERTISING_EVT ) == 0 ) ){
                    VOID osal_set_event( gapRole_TaskID, START_ADVERTISING_EVT );// Start advertising
                }
            }
            if ( pPkt->hdr.status != SUCCESS ){                
                gapRole_state = GAPROLE_ERROR;// Set into Error state
                notify = TRUE;
            }
        }
        break;

        case GAP_MAKE_DISCOVERABLE_DONE_EVENT://0x03 //!<当发出可发现的请求完成时发送
        case GAP_END_DISCOVERABLE_DONE_EVENT:{//0x04 //!<广告结束时发送
					LOG("-->[ GAP_END_DISCOVERABLE_DONE_EVENT ]  pMsg->opcode:%02X\r\n",pMsg->opcode);
            ...
            if ( pPkt->hdr.status == SUCCESS ){
                if ( pMsg->opcode == GAP_MAKE_DISCOVERABLE_DONE_EVENT ){
                    ...
                    if ( gapRole_AdvertOffTime != 0 ){
                        if ( ( gapRole_AdvEnabled ) ){VOID osal_start_timerEx( gapRole_TaskID, START_ADVERTISING_EVT, gapRole_AdvertOffTime );}
                    }else{
                    ...
                    if (gapRole_state == GAPROLE_CONNECTED_ADV){
                        //广告关闭期间 In the Advertising Off period
                        gapRole_state = GAPROLE_CONNECTED;
                        ...
                        //重新开启广告。 Turn advertising back on.
                        VOID osal_set_event( gapRole_TaskID, START_ADVERTISING_EVT );
                    }else{
                        gapRole_state = GAPROLE_WAITING;// In the Advertising Off period
                    }
                }
            }else{gapRole_state = GAPROLE_ERROR;}
            notify = TRUE;
        }
        break;

        case GAP_LINK_ESTABLISHED_EVENT:{//0x05 //!<当建立链接请求完成时发送
					LOG("-->[ GAP_LINK_ESTABLISHED_EVENT ]  pMsg->opcode:%02X\r\n",pMsg->opcode);
            ...
            if ( pPkt->hdr.status == SUCCESS ){
                ...
                if ( gapRole_RSSIReadRate ){
                    VOID osal_start_timerEx( gapRole_TaskID, RSSI_READ_EVT, gapRole_RSSIReadRate );// Start the RSSI Reads
                }
        ...
        case GAP_LINK_TERMINATED_EVENT:{//0x06 //!<0x06 连接终止时发送
					LOG("-->[ GAP_LINK_TERMINATED_EVENT ]  pMsg->opcode:%02X\r\n",pMsg->opcode);
        ...
        case GAP_LINK_PARAM_UPDATE_EVENT:{//0x07 //!<收到更新参数事件时发送
					LOG("-->[ GAP_LINK_PARAM_UPDATE_EVENT ]  pMsg->opcode:%02X\r\n",pMsg->opcode);
...
//AT_LOG(" SecondAdvEnable notify %d,pMsg->opcode %d,gapRole_state %d\n ",notify,pMsg->opcode,gapRole_state);
    ...
    if ( notify == TRUE ){
        //通知应用程序新的状态变化 Notify the application with the new state change
        if (pGapRoles_AppCGs && pGapRoles_AppCGs->pfnStateChange ){pGapRoles_AppCGs->pfnStateChange( gapRole_state );}
    }
}
