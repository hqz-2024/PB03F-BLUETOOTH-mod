...
//    uint16 EstMIN = DEFAULT_DESIRED_MIN_CONN_INTERVAL;
    GAP_SetParamValue( TGAP_CONN_EST_INT_MIN, MIN_CONN_INTERVAL );//  * 1.25ms  使用连接建立过程时的最小链路层连接间隔
    GAP_SetParamValue( TGAP_CONN_EST_INT_MAX, MAX_CONN_INTERVAL );//使用连接建立过程时的最大链路层连接间隔	
    GAP_SetParamValue( TGAP_CONN_EST_SUPERV_TIMEOUT,DEFAULT_UPDATE_CONN_TIMEOUT );//连接建立过程时，链路层连接监督超时
