...
#define BLE_NameLen  3
unsigned char BleName[BLE_NameLen] = "BYS";
...
void BLE_Application_gen_AdvData(unsigned char *Dat,unsigned char Len){
	...
	AdvData[AdvDataLen++] = BLE_NameLen+1;
	AdvData[AdvDataLen++] = 0x09;
	for (uint8 i = 0; i < BLE_NameLen; i++){AdvData[AdvDataLen++] = BleName[i];}
	if(Len && Dat!=NULL){
		if(Len>24-BLE_NameLen)Len = 24-BLE_NameLen;
		AdvData[AdvDataLen++] = Len+1;
		AdvData[AdvDataLen++] = 0xFF;
		for (uint8 i = 0; i < Len; i++){AdvData[AdvDataLen++] = Dat[i];}
	...
	if(Peripheral_Broadcasting_Start_Num){
		if(SW==TRUE){
			...
			if(ADVDAT_Renew_Sta==TRUE){
				ret = GAP_UpdateAdvertisingData(BLE_Peripheral_TaskID,TRUE, AdvDataLen, AdvData);//更新广播数据
//				ret = GAP_UpdateAdvertisingData(BLE_Peripheral_TaskID,FALSE, ScanRspLen, ScanRspData);//更新广播响应数据
//				ADVDAT_Renew_Sta=FALSE;
...
unsigned char BLE_Peripheral_GAP_MAKE_DISCOVERABLE_DONE_EVENT_Call(void){
		VOID osal_start_timerEx(BLE_Peripheral_TaskID,BLE_ADVERTISING_TIMEOUT_EVT,ADV_INTERVAL_DEF_625US);//等待蓝牙连接超时事件
	return SUCCESS;
}
/*-----------------------------------------------------------------------------------------------------------------------------------*/
...
//		VOID osal_memcpy( gapRole_ConnectedDevAddr, pPkt->devAddr, B_ADDR_LEN );
		...
		updateReq.intervalMin = BLE_ConnInterval;
		updateReq.intervalMax = BLE_ConnInterval*3;
		updateReq.slaveLatency = BLE_ConnLatency;
		updateReq.timeoutMultiplier = BLE_ConnTimeout;
		...
		LOG("MinConnectionInterval : %d\r\n",updateReq.intervalMin);
		LOG("MaxConnectionInterval : %d\r\n",updateReq.intervalMax);
...
void BLE_Peripheral_GAP_LINK_PARAM_UPDATE_EVENT(void){
	...
	l2capParamUpdateReq_t updateReq = {BLE_ConnInterval,BLE_ConnInterval*3,BLE_ConnLatency,BLE_ConnTimeout};
...
void BLE_Peripheral_Start_Init(unsigned char taskId){
	...
	BLE_Application_gen_AdvData(NULL,0);
...
//		GAPRole_SetParameter(GAPROLE_TIMEOUT_MULTIPLIER, sizeof(uint16), &desired_conn_timeout);
    ...
    GGS_SetParameter(GGS_DEVICE_NAME_ATT, BLE_NameLen, BleName);
    // Set advertising interval
    {
        uint16_t advInt = ADV_INTERVAL_DEF_625US;
