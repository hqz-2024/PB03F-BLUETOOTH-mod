...
/*********************************************************************
    LOCAL FUNCTIONS
*/
static uint8 bleuart_ReadAttrCB( uint16 connHandle, gattAttribute_t* pAttr,uint8* pValue, uint8* pLen, uint16 offset, uint8 maxLen );
...
*/
unsigned char BYS_BLE_Service_AddService( bleuart_ProfileChangeCB_t cb){
    ...
    status = GATTServApp_RegisterService( bleuart_ProfileAttrTbl,GATT_NUM_ATTRS( bleuart_ProfileAttrTbl ),&bleuart_ProfileCBs );
    if(status!=SUCCESS)LOG("Add rawpass service failed!\n");
...
static uint8 bleuart_ReadAttrCB( uint16 connHandle, gattAttribute_t* pAttr,uint8* pValue, uint8* pLen, uint16 offset, uint8 maxLen ){
    ...
    if ( osal_memcmp(pAttr->type.uuid, clientCharCfgUUID, 2)){
        ...
        *pLen = 2; osal_memcpy(pValue, pAttr->pValue, 2);
...
static bStatus_t bleuart_WriteAttrCB( uint16 connHandle, gattAttribute_t* pAttr,uint8* pValue, uint8 len, uint16 offset ){
    ...
    }else if(osal_memcmp(pAttr->type.uuid, bleuart_PTCharUUID, 2) && (pAttr->handle == bleuart_ProfileAttrTbl[RAWPASS_VALUE_HANDLE].handle)){
        if(bleuart_AppCBs){
            evt.connHandle = connHandle;
            ...
            evt.data = pValue;
            bleuart_AppCBs(&evt);
					BYS_BLE_Service_Notify(pValue,(uint16_t)len);
        }
    }else{return ( ATT_ERR_ATTR_NOT_FOUND );}
    return ( status );
}
...
uint8 BYS_BLE_Service_NotifyIsReady(void){return (bleuart_TxCCCD.value == GATT_CLIENT_CFG_NOTIFY);}
void BYS_BLE_Service_set_Notify(void){    bleuart_TxCCCD.value |= GATT_CLIENT_CFG_NOTIFY;}
void BYS_BLE_Service_clear_Notify(void){    bleuart_TxCCCD.value = 0;}
...
/*********************************************************************
    ...
    @return      Success or Failure
*/
unsigned char BYS_BLE_Service_Notify(unsigned char *Dat,unsigned short Len){
    // If notifications enabled
  if ( bleuart_TxCCCD.value & GATT_CLIENT_CFG_NOTIFY ){
...
//	size = ((pctx->rx_size - pctx->rx_offset) > 244) ? 244 : pctx->rx_size - pctx->rx_offset; // ATT_MTU_SIZE  20  192
		memcpy((unsigned char *)&pNoti.value,Dat,Len);
		pNoti.len = Len;
		HAL_EXIT_CRITICAL_SECTION();
        // Set the handle
    pNoti.handle = bleuart_ProfileAttrTbl[RAWPASS_VALUE_HANDLE].handle;
//    LOG("-->>%s [%04X] Len=%d\r\n",__func__,evt.connHandle,Len);       
    return GATT_Notification(evt.connHandle, &pNoti, FALSE); // Send the Indication
  }
