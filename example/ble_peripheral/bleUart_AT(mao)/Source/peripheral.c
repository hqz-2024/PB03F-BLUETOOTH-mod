...
//	LOG("-->%s  events:%04X\r\n",__func__,pMsg->event);
    ...
    case HCI_GAP_EVENT_EVENT:
        ...
        if ( pMsg->status == HCI_COMMAND_COMPLETE_EVENT_CODE ){
            ...
            if ( pPkt->cmdOpcode == HCI_LE_SET_PHY ){AT_LOG("[HCI PHY] %2x \n",pPkt->cmdStatus);}
...
//                          AT_LOG("[HCI PHY EVT]s%d r%d t%d\n",pkt->status, pkt->rxPhy,pkt->txPhy);
    ...
    case GAP_MSG_EVENT:        gapRole_ProcessGAPMsg( (gapEventHdr_t*)pMsg );        break;
...
static void gapRole_ProcessGAPMsg( gapEventHdr_t* pMsg )
{
    ...
    if( LegacyAdvEnable )
    {
        ...
        case GAP_LINK_TERMINATED_EVENT:
        {
            ...
            if ( gapRole_AdvEnabled)
            {
                // End advertising so we can restart advertising in order to change to connectable advertising from nonconnectable.
                VOID GAP_EndDiscoverable( gapRole_TaskID );
            }
        ...
        case GAP_LINK_PARAM_UPDATE_EVENT:
        {
            ...
            if ( pPkt->hdr.status == SUCCESS )
            {
                ...
                if ( osal_get_timeoutEx( gapRole_TaskID, START_CONN_UPDATE_EVT ) == 0 )
                {
                    ...
                    {
                        (*pGapRoles_ParamUpdateCB)( gapRole_ConnInterval,gapRole_ConnSlaveLatency,gapRole_ConnTimeout );
                    }
                }
            }
        }
...
bStatus_t GAPRole_SendUpdateParam( uint16 minConnInterval, uint16 maxConnInterval,uint16 latency, uint16 connTimeout, uint8 handleFailure ){
    ...
    if ( gapRole_state != GAPROLE_CONNECTED ){return ( bleNotConnected );}
