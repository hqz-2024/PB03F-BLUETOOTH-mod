...
void at_update_bd_addr(void)
{
    ...
    *(p_ownPublicAddr++)              = UID[5];
    *(p_ownPublicAddr++)              = UID[4];
    *(p_ownPublicAddr++)              = UID[3];
    *(p_ownPublicAddr++)              = UID[2];
    *(p_ownPublicAddr++)              = UID[1];
    *(p_ownPublicAddr++)              = UID[0];
...
#endif
    hal_uart_init(cfg, (UART_INDEX_e)get_uart_idx(0));
#ifdef BLEUART_DEDICATE
...
void at_Init()
{
    hal_uart_deinit((UART_INDEX_e)get_uart_idx(0));
    at_uart_init();
