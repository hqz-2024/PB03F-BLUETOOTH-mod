...
// uint8_t at_flag  = 0;
uint8_t g_uart_idx = UART0; // UART0
// 新增的用户自定义串口索引
uint8_t g_use_uart_idx = UART1; // UART1
...
uint8_t get_uart_idx(uint8_t idx)
{
    uint8_t uart_idx;
    switch (idx) {
        case UART0:
...
int BUP_data_BLE_to_uart_send(void)
{
    ...
    if (pctx->tx_state != BUP_TX_ST_IDLE && pctx->tx_size) {
        ...
        hal_uart_send_buff((UART_INDEX_e)g_use_uart_idx, pctx->tx_buf, pctx->tx_size);
        pctx->tx_size  = 0;
...
int BUP_data_BLE_to_uart(uint8_t *pdata, uint8_t size)
{
    ...
    BUP_ctx_t *pctx = &mBUP_Ctx;
    // LOG_DEBUG("BUP_data_BLE_to_uart: tx_state = %d, tx_size = %d", pctx->tx_state, pctx->tx_size);
    // AT_LOG("t_1");
    switch (pctx->tx_state) {
        case BUP_TX_ST_IDLE: {
            ...
            //BUP_data_BLE_to_uart_send();               //2025-11-17 增加修改
            
            osal_stop_timerEx(bleuart_TaskID, BUP_OSAL_EVT_USER_UART_TX);
            osal_start_timerEx(bleuart_TaskID, BUP_OSAL_EVT_USER_UART_TX, USE_Query_Pause/2);
            // 延迟 20ms 再发送到 UART：通过启动 BUP_OSAL_EVT_BLE_TIMER 事件来异步触发发送
            osal_start_timerEx(bleuart_TaskID, BUP_OSAL_EVT_BLE_TIMER, 20);
            break;
        }
...
int BUP_data_uart_to_BLE_send(void)
{
    ...
    if (pctx->rx_state != BUP_RX_ST_IDLE && pctx->rx_size) {
        ...
        //LOG_DEBUG("bleuart_NotifyIsReady: %d\n", bleuart_NotifyIsReady());

        // if(bleuart_NotifyIsReady() == FALSE)  //要屏蔽
        // {
        //     BUP_connect_handler();
        ...
        while (1) {
            ...
            // 这里增加用户自定义函数接口
            // UART1接收解析（在接收回调中调用）
            uart1_parse_pkt(notify_data.value, notify_data.len);
            ret = SUCCESS;
            //原有BLE的发送
...
int BUP_disconnect_handler(void)
{
    ...
    // 因为use也要用到该串口，可能会有数据发送过来，所以这里不清零
...
int BUP_init()
{
    ...
    // cfg.baudrate = at_get_baudrate();  //屏蔽默认AT的波特率

    hal_uart_deinit((UART_INDEX_e)g_use_uart_idx);
    hal_uart_init(cfg, (UART_INDEX_e)g_use_uart_idx);
...
#endif
    hal_uart_set_tx_buf((UART_INDEX_e)g_use_uart_idx, pctx->hal_uart_tx_buf, UART_TX_BUF_SIZE);
    AT_LOG("PT Mod\n");
