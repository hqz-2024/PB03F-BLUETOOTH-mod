...
//#define BUP_OSAL_EVT_USER_UART_RX                         0x4000  //
#define BUP_OSAL_EVT_USER_UART_TX                         0x4000  //
                                                          //0x8000  //已经被占用
...
#define FLOW_CTRL_IO_HOST_WAKEUP      P10 //host mcu wakeup befor host-->uart-->620x
//
#define LED_GPIO_PIN                  P18 //BLE连接指示灯
#define LED_RED_GPIO_PIN              P7  //收发数据指示灯
#define LED_GREEN_GPIO_PIN            P11 //收发数据指示灯
...
extern uint16 uart_Txing;

//LED_DEBUG
#define LED_DEBUG             1

//想在BLE连接后还有广播功能，要主动再使能一次
#define Broad_Connected       1

//连接后修改查询的设备类型或上(0x8000),为0x0000表示不修改
#define CMD_Query_Connect     0x8000

//主动查询设备状态
#define USE_Query_Time        100   // 查询间隔时间
#define USE_Query_Pause       2000  // 查询暂停时间

// UART1 数据包定义 
#define UART1_PKT_HEADER      0x55AA
#define UART1_PKT_TAIL        0x55BB
#define UART1_DEVICE_TYPE     0x0000
//#define UART1_CMD_QUERY       0x0002
#define UART1_MAX_DATA_LEN    1

typedef enum
{
    QUERY_CMD_Mate = 0x0002, //查询材料
    QUERY_CMD_T2_4T,  //2t_4T
    QUERY_CMD_Current,  //电流
    QUERY_CMD_Gas,  //气体
    QUERY_CMD_ARC,  //弧
    QUERY_CMD_P_M,  //P_M
    QUERY_CMD_Warning,  //Warning
    QUERY_CMD_Voltage,  //Voltage
} USE_QUERY_CMD_E;

typedef struct {
    uint16_t header;//包头：0x55AA
    uint16_t dev_type;//设备类型;
    uint16_t cmd;//包类型;命令码
    uint16_t data;//[UART1_MAX_DATA_LEN];//包数据
    //uint16_t data_len;
    uint16_t checksum;//校验 = cmd + data;
    uint16_t tail;//包尾：0x55BB
} uart1_pkt_t;//数据包结构体
...
typedef struct
{
    ...
    uint16_t dev_type;//设备类型
    uint16_t mate_type;//材料类型
    uint16_t T2_4T;  //2T || 4T
    uint16_t current; //电流
    uint16_t gas; //气体
    uint16_t arc; //弧
    uint16_t P_M; //P || M
