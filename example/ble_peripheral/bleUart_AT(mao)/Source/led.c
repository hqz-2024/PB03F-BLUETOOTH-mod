...
/*********************************************************************
    GLOBAL VARIABLES
*/
GPIO_Pin_e led_Pin,led_R_Pin,led_G_Pin;
...
void led_R_initial(GPIO_Pin_e pin)
{
    led_R_Pin = pin;
    hal_gpio_pin_init(led_R_Pin, OEN);
    hal_gpio_write(led_R_Pin, 0);
    hal_gpio_pull_set(led_R_Pin, PULL_DOWN);
}

void led_G_initial(GPIO_Pin_e pin)
{
    led_G_Pin = pin;
    hal_gpio_pin_init(led_G_Pin, OEN);
    hal_gpio_write(led_G_Pin, 0);
    hal_gpio_pull_set(led_G_Pin, PULL_DOWN);
}
/*********************************************************************
    @fn      led_set_status
...
void led_R_set_status(LED_STATUS_e led_status)
{
    switch(led_status)
    {
    case LED_STATUS_OFF:
    ...
    case LED_STATUS_ON:
        hal_gpio_write(led_R_Pin, 1);
        hal_gpio_pull_set(led_R_Pin, STRONG_PULL_UP); //STRONG_PULL_UP / WEAK_PULL_UP
        set_led(LED_PWR_ON);
        break;
...
void led_G_set_status(LED_STATUS_e led_status)
{
    switch(led_status)
    {
    case LED_STATUS_OFF:
    ...
    case LED_STATUS_ON:
        hal_gpio_write(led_G_Pin, 1);
        hal_gpio_pull_set(led_G_Pin, STRONG_PULL_UP); //STRONG_PULL_UP / WEAK_PULL_UP
        set_led(LED_PWR_ON);
        break;
