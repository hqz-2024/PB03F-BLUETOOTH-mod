...
#if(USE_RFLIGHT)
extern void init_config_light(void);

extern llStatus_t LL_InitConnectContext_light(llConnState_t*    pConnContext,
                                 uint8* pConnBuffer,
                                 uint8 maxConnNum,
                                 uint8 maxPktPerEventTx,
                                 uint8 maxPktPerEventRx,
                                 uint8 blePktVersion);
extern void ll_patch_multislave_light(void);
#else
...
static void ble_mem_init_config(void)
{
    ...
    LL_InitConnectContext(pConnContext,
                          ...
                          BLE_PKT_VERSION);
#else
    LL_InitConnectContext_light(pConnContext,
                          g_pConnectionBuffer,
                          BLE_MAX_ALLOW_CONNECTION,
                          BLE_MAX_ALLOW_PKT_PER_EVENT_TX,
                          BLE_MAX_ALLOW_PKT_PER_EVENT_RX,
