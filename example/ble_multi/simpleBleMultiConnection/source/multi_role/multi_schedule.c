 ...
 *******************************************************************************************************/
#define MULTI_SCH_ADV_DURATION_TIME     5 //ms
...
multi_init_timeout_t g_multi_init_timeout = {0};
...
uint8_t multiRole_findScanNode( void )
{
    uint8_t len = 0;
    multiScehdule_t* node = multiRole_search_phead( MULTI_SCH_MODE );
...
uint8_t multiRole_findInitNode( void )
{
    uint8_t len = 0;
    multiScehdule_t* node = multiRole_search_phead( MULTI_SCH_MODE );
...
uint8_t multiRole_findAdvNode_num( void )
{
    uint8_t len = 0;
    multiScehdule_t* node = multiRole_search_phead( MULTI_SCH_MODE );
...
void restart_multi_schedule_process(void)
{
...
#if (MAX_CONNECTION_MASTER_NUM > 0 )
void multi_role_add_scan_node(void)
{
    if(multiRole_findScanNode() == 0 && (multiLinkGetMasterConnNum()+multiRole_findInitNode()) < MAX_CONNECTION_MASTER_NUM)
    {
        muliSchedule_config( MULTI_SCH_SCAN_MODE, 0x01 );
    }
    else if(multiRole_findBusyNode() == NULL)
    {
        restart_multi_schedule_process();
    }
}
void multi_role_add_init_node(void)
{
    if(multiRole_findInitNode() == 0 && (multiLinkGetMasterConnNum()+multiRole_findScanNode()) < MAX_CONNECTION_MASTER_NUM)
    {
...
void multiConfigSchScan_param(void)
{
    ...
    if( node != NULL && (node->role == scanner) )
    {
        node->busy = FALSE;
...
static void MultiScheduleInitating(multiScehdule_t* node)
{
    ...
    if( lnode && node->roleScd.initiate.initiating == false )
    {
...
//          LOG( bdAddr2Str( lnode->addr ) );
            ...
            if(osal_memcmp(g_multi_init_timeout.conn_addr, lnode->addr, 6) == 0)
            {
                osal_memcpy(g_multi_init_timeout.conn_addr, lnode->addr, 6);
                g_multi_init_timeout.conn_start_time = osal_getClock();
            }
