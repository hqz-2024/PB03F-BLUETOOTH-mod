...
//    printf("__rcv 0x%04X 0x%04X 0x%02X\n",call_receive_para.heartbeat_addr,call_receive_para.heartbeat_subnet_index,call_receive_para.heartbeat_countlog);
    ...
    MS_PACK_LE_2_BYTE_VAL(&buffer[marker], MS_STATE_VENDORMODEL_HB_CALLBACK_T);
    marker += 2;
...
//        printf("%02X\n",buffer[i]);
//    }
    opcode = MS_ACCESS_VENDORMODEL_WRITECMD_OPCODE;
    /* Publish - reliable */
    pdu_ptr = buffer;
//    if(cfg_retry_flag == 0)
//    {
...
//    UCHAR   proxy_state;
    ...
    MS_access_cm_set_transmit_state(MS_NETWORK_TX_STATE, (0<<3)|4);
}
