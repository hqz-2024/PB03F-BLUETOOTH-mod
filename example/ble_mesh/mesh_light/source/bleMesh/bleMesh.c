...
// GAP GATT Attributes
static const uint8 attDeviceName[GAP_DEVICE_NAME_LEN] = "PHYPLUS MSH LIGHT";

static gapAdvertisingParams_t bleMesh_advparam;

UCHAR cmdstr[CLI_MAX_ARGS];
UCHAR cmdlen;
API_RESULT cli_ext_adv_test(UINT32 argc, UCHAR* argv[])
{
    UCHAR buff[245];
    ...
    buff[2] = 0x62;
    buff[3] = 0x22;
    blebrr_ext_adv_send(buff,sizeof(buff));
    return API_SUCCESS;
}
...
DECL_CONST CLI_COMMAND cli_cmd_list[] =
{
    ...
    /*heartbeat set */
    { "extadvtst", "extadv test", cli_ext_adv_test },
...
void bleMesh_Init( uint8 task_id )
{
    ...
    GAP_CentDevMgrInit(0x20);
    //ext adv recv
    GAP_CentDevExtMgrInit();
    GAP_PeriDevMgrInit();
...
//        printf ("ED Status - %d", pMsg->hdr.status);
    ...
    case GAP_DEVICE_DISCOVERY_EVENT:    //scan start/stop
        if (SUCCESS == pMsg->hdr.status)
        {
            GAP_DeviceDiscoveryRequestMesh(bleMesh_TaskID);
        }
...
)
{
    ...
    GAP_SetParamValue( TGAP_GEN_DISC_SCAN, 30 );
...
)
{
    ...
    if (0x00 != scan_enable)
    {
        ret = GAP_DeviceDiscoveryRequestMesh(bleMesh_TaskID);
    }
    else
    {
